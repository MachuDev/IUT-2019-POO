...
{
    ptr -> SetNextNode (std::shared_ptr<CNode<T>> (new CNode<T> (val, ptr-> GetNextNode())));
...
{
    std::shared_ptr<CNode<T>> ptrTmp = std::shared_ptr<CNode<T>>(new CNode<T> (val, ptr -> GetNextNode ()));
...
template <typename T>
CList<T>::CList () : m_fictionaHead (std::shared_ptr<CNode<T>> (new CNode<T>())), m_realTail (m_fictionaHead) {};
...
{
    std::shared_ptr<CNode<T>> pTmp = std::shared_ptr<CNode<T>>(new CNode<T> (val, NULL));
